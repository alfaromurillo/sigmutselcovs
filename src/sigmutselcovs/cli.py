...
def _add_project_arg(parser):
    parser.add_argument("project",
                        help="TCGA study code (e.g. COAD, BRCA)")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_project_arg(p)
    _add_data_dir_arg(p)
    p.add_argument("--force-generation", action="store_true",
                   help="recompute the per-gene caches")
    p.add_argument("--include", nargs="+", default=None,
                   choices=["gtex", "gexp", "repliseq", "roadmap",
                            "atac"])
    ...
    p = sub.add_parser("fetch",
                       help="download pre-built matrices from OSF")
    _add_project_arg(p)
    p.add_argument("--which", default="pca",
                   choices=["pca", "full", "simple", "tcga"])
    p.add_argument("--dest", default=None)
    p.add_argument("--force", action="store_true")

    p = sub.add_parser("check-updates",
                       help="compare sources to recorded state")
    p.add_argument("--update-file", action="store_true",
                   help="rewrite known states in sources.json")
    p.add_argument("--sources", nargs="+", default=None)

    p = sub.add_parser("download-gtex",
                       help="fetch the GTEx GCT into the user cache")
    p.add_argument("--force", action="store_true")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "projects":
        ...
        for code, spec in sorted(load_registry().items()):
            sources = [s for s in ("gtex", "gexp", "repliseq",
                                   "roadmap", "atac")
    ...
    if args.command == "download":
        ...
        report = download_covariates(
            args.project, args.data_dir, which=tuple(args.which),
            force=args.force, dry_run=args.dry_run)
        print(report.summary())
        failed = any(info["status"] == "failed"
    ...
    if args.command == "build":
        ...
        matrices = build_covariate_matrix(
            args.project, args.data_dir,
            force_generation=args.force_generation,
            include=args.include, exclude=tuple(args.exclude),
            cache_matrices=not args.no_cache,
            strict_validation=args.strict)
        for name in ("full", "simple", "tcga"):
            shape = getattr(matrices, name).shape
            print(f"cov_matrix_{name}: {shape[0]} genes x "
                  f"{shape[1]} columns")
        return 0

    if args.command == "validate":
        from .validate import print_validation_report, validate_covariates
        frame = validate_covariates(args.project, args.data_dir,
                                    strict=False)
    ...
    if args.command == "fetch":
        ...
        frame = fetch_covariate_matrix(args.project, args.which,
                                       dest=args.dest,
                                       force=args.force)
        print(f"cov_matrix_{args.which}: {frame.shape[0]} genes x "
              f"{frame.shape[1]} columns")
        return 0

    if args.command == "check-updates":
        from .updates import check_updates, print_update_report
        frame = check_updates(sources=args.sources,
