...
def _selected(
    include: Sequence[str] | None, exclude: Sequence[str]
) -> set[str]:
...
def _load_repliseq(
    spec, paths: ProjectPaths, gtf: str | Path, force_generation: bool
) -> list[pd.DataFrame]:
    ...
    if spec.type == "mat":
        ...
        if missing and not cache.exists():
            _skip(
                "repliseq",
                f"missing bigWigs {missing} and no "
                f"cache at {cache}",
            )
            return []
        if spec.type == "wavelet":
...
def _atac_prefix(spec, atac_covs: pd.DataFrame) -> str:
    ...
    fallback = (
        stems[0]
        if stems and all(s == stems[0] for s in stems)
        else prefix
    )
    logger.warning(
        "No ATAC column starts with %r; falling back to prefix %r "
        "derived from the bigWig names",
        prefix + "_",
        fallback,
    )
    return fallback
...
def _describe_block(
    source: str, spec: ProjectSpec, frame: pd.DataFrame
) -> dict:
    """Block-level constants for the column dictionary."""
    if source == "gtex":
        return {
            "description": "GTEx median gene expression",
    ...
    if source == "gexp_mean":
        return {
            "description": "Mean TCGA gene expression over "
    ...
    if source == "gexp_per_sample":
        return {
            "description": "Per-sample TCGA STAR expression "
    ...
    if source == "atac":
        return {
            "description": "TCGA ATAC-seq insertions, mean over "
    ...
    if source == "roadmap":
        return {
            "description": "Roadmap histone-mark ChIP fold-change "
...
def _write_column_dictionary(
    ...
    for source, columns in blocks:
        ...
        for column in columns:
            ...
            transform = (
                "; ".join(f"log({c:.6g} + x)" for c in transforms)
                if transforms
                else "none"
            )
            rows.append(
    ...
    logger.info(
        "Wrote column dictionary (%d rows) to %s",
        len(frame),
        paths.column_dictionary_csv,
    )
...
def _write_manifest(
    ...
    manifest = {
        ...
        "shapes": {
            name: list(getattr(matrices, name).shape)
    ...
    *,
    ...
    if "gexp" in selected:
        if spec.gexp is None:
            ...
            metrics = (
                list(spec.gexp.metrics)
                if spec.gexp.metrics is not None
                else None
            )
            ...
            blocks.append(
                ("gexp_per_sample", list(per_sample.columns))
            )
            included.add("gexp")
    ...
    if "atac" in selected:
        if spec.atac is None:
            _skip(
                "atac", f"not defined in the registry for {spec.code}"
            )
        else:
            atac_covs = _load_chromatin(
                "atac",
                paths.atac_covs_csv,
                paths.atac_bigwig_files(),
                gtf_for(spec.atac.assembly),
                force_generation,
            )
    ...
    if "roadmap" in selected:
        if spec.roadmap is None:
            ...
            roadmap_covs = _load_chromatin(
                "roadmap",
                paths.roadmap_covs_csv,
                paths.roadmap_bigwig_files(),
                gtf_for(spec.roadmap.assembly),
                force_generation,
            )
    ...
    if not frames:
        raise ValueError(
            f"No covariate sources available for "
    ...
    duplicated = cov_matrix_full_raw.columns[
        cov_matrix_full_raw.columns.duplicated()
    ].unique()
    if len(duplicated):
        logger.warning(
            "Duplicated covariate columns after concat: %s",
            list(duplicated),
        )
    ...
    if sm.gtex_column in cov_matrix_full.columns:
        ...
        logger.warning(
            "cov_matrix_simple: %r not in the full matrix "
    ...
    if sm.include_mrt:
        if "mrt" in cov_matrix_full.columns:
            ...
            logger.warning(
                "cov_matrix_simple: no mrt column; omitting"
            )
    body_cols = [
        c for c in cov_matrix_full.columns if sm.body_pattern in c
    ]
    if body_cols:
        simple_cols["h3k4me3_body"] = cov_matrix_full[body_cols].mean(
            axis=1
        )
    else:
        logger.warning(
            "cov_matrix_simple: no columns match %r; omitting",
            sm.body_pattern,
        )
    promoter_cols = [
        c for c in cov_matrix_full.columns if sm.promoter_pattern in c
    ]
    if promoter_cols:
        simple_cols["h3k9me3_prom"] = cov_matrix_full[
            promoter_cols
        ].mean(axis=1)
    else:
        logger.warning(
    ...
    if "tpm_unstranded" in cov_matrix_full.columns:
        ...
        logger.warning(
            "cov_matrix_tcga: no tpm_unstranded column; "
            "omitting gexp"
        )
    if atac_covs is not None and spec.atac is not None:
        ...
        for suffix, name in (
            ...
            cols = [
                c
                for c in cov_matrix_full.columns
            ...
            if cols:
                ...
                logger.warning(
                    "cov_matrix_tcga: no %s* columns ending "
                    "in %s; omitting %s",
                    prefix,
                    suffix,
                    name,
    ...
    if cache_matrices:
        ...
        _write_manifest(
            paths, spec, included, matrices, registry_path
        )
        _write_column_dictionary(
            paths,
            spec,
            blocks,
        ...
        logger.info(
            "Cached covariate matrices under %s", paths.matrices_dir
        )
